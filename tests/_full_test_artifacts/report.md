...
- 真实技能目录总数：**155**（带可执行脚本 **121**、纯 prompt 型 **34**）
- 第二代冒烟测试文件：**121** = 手搓强测试 **28** + `gen_smoke.py` 自动生成的弱测试 **93**
- **真实技能冒烟实测：121 / 121 全部 pass（offline=0、dep=0、0 警告）** ✅
- 自动生成工具：`tools/gen_smoke.py`（`--force` 可覆盖重生成；已修模板 `SyntaxWarning` 残留）
- 详细清单见 `tests/_full_test_artifacts/skill_smoke_report.md`

**这才是技能包质量的真指标**：所有 121 个脚本化技能都 ship 了冒烟测试，离线全绿，零警告。第一代的高 pass 率有"自嗨"成分（测的是我手搓的代理），第二代才是硬门禁。两者互补：

| 维度 | 第一代（代理重实现） | 第二代（真实技能冒烟） |
|---|---|---|
| 测的对象 | 20 个合成技能名（≠真实技能） | 121 个真实技能自带测试 |
| 结果 | 19 pass / 1 warn | 121 pass / 0 fail |
| 能证明 | "交付物可生成" | "技能脚本真能跑" |
| 覆盖演进 | 与 117 个真实技能脱钩 | P0 扩展前覆盖 28 → 现覆盖 121（全脚本化技能） |

**剩余真问题（诚实列出，2026-09-20 P0 扩展后）：**
1. **93 个弱冒烟测试**（自动生成的 `test_smoke_all.py`）只验证「import 不崩 + `--help` 不崩」，未验证真实语义 → 应逐步升级为手搓强测试（真输入 + 真断言）。这是 P0 之后的持续增量项，非阻塞项。
2. **34 个纯 prompt 型技能**无脚本可冒烟 → 质量靠 SKILL.md 评审（P2），超出脚本冒烟范围。
3. 生成式媒体保真度受算力/网关所限 → P1 model_route（已落地 helper，离线兜底=可移植性特性）。
