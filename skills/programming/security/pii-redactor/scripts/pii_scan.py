...
RE_PLATE = re.compile(
    r"([" + PROV + r"][A-Z])\d{4,5}[A-Z0-9\u6302\u5b66\u8b66\u6e2f\u4f7f\u9886]"
)
RE_ADDR = re.compile(
    r"("
    + PROV
    + r"[\u7701\u5e02]?[\u4e00-\u9fa5]{2,20}(\u8def|\u8857|\u9053|\u5cf0|\u53f7)[\u4e00-\u9fa5\d]{0,20})"
)
NAME_KEYS = re.compile(
    r"(\u59d3\u540d|\u8054\u7cfb\u4eba|\u5ba2\u6237\u540d|\u7528\u6237\u5b9e\u540d|applicant|full[_]?name)\s*[:\uff1a=]\s*([\u4e00-\u9fa5A-Za-z\u00b7]{2,15})"
)
...
def main():
    ...
    ap.add_argument(
        "--dry-run",
        action="store_true",
        help="report hits only, write nothing (default; kept for compat)",
    )
    ap.add_argument(
        "--execute", action="store_true", help="actually write the redacted output"
    )
    ...
    a = ap.parse_args()
    # default is dry-run; only --execute (without --dry-run) writes
    ...
    if not a.dry_run and a.out is None:
        ...
        if a.out is None:
            ...
            sys.exit(2)
    cats = (
        [c.strip() for c in a.categories.split(",") if c.strip()]
        if a.categories
    ...
    for f in files:
        ...
        if not a.dry_run:
            redact_buf.append(
                "".join(
                    apply_hits(l, scan_line(l, cats, a.no_heuristics), a.strategy)
                    for l in lines
                )
            )
    if a.dry_run:
