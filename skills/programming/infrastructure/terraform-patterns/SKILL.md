...
## CI/CD & Advanced Patterns (extracted)

CI/CD pipelines (GitHub Actions plan/apply, drift detection), proactive red flags,
multi-cloud provider aliasing, OpenTofu migration, Infracost cost gates,
importing existing infrastructure, and Terragrunt layouts now live in:

`references/cicd-and-advanced-patterns.md` — read it when the task touches any of those topics.
