...
- [JOIN Patterns](#join-patterns)
  ...
  - [LEFT JOIN — all rows from left, matching from right](#left-join-all-rows-from-left-matching-from-right)
  - [Self JOIN — comparing rows within the same table](#self-join-comparing-rows-within-the-same-table)
  - [CROSS JOIN — every combination (cartesian product)](#cross-join-every-combination-cartesian-product)
  - [LATERAL JOIN (PostgreSQL) — correlated subquery as a table](#lateral-join-postgresql-correlated-subquery-as-a-table)
- [Common Table Expressions (CTEs)](#common-table-expressions-ctes)
  - [Basic CTE — readable subquery](#basic-cte-readable-subquery)
  - [Multiple CTEs — chaining transformations](#multiple-ctes-chaining-transformations)
  - [Recursive CTE — hierarchical data](#recursive-cte-hierarchical-data)
  - [Recursive CTE — path traversal](#recursive-cte-path-traversal)
- [Window Functions](#window-functions)
  - [ROW_NUMBER — assign unique rank per partition](#rownumber-assign-unique-rank-per-partition)
  - [RANK and DENSE_RANK — handle ties](#rank-and-denserank-handle-ties)
  - [Running total and moving average](#running-total-and-moving-average)
  - [LAG / LEAD — access adjacent rows](#lag-lead-access-adjacent-rows)
  - [NTILE — divide into buckets](#ntile-divide-into-buckets)
  - [FIRST_VALUE / LAST_VALUE](#firstvalue-lastvalue)
- [Subquery Patterns](#subquery-patterns)
  - [EXISTS — correlated existence check](#exists-correlated-existence-check)
  - [NOT EXISTS — safer than NOT IN for NULLs](#not-exists-safer-than-not-in-for-nulls)
  - [Scalar subquery — single value](#scalar-subquery-single-value)
  - [Derived table — subquery in FROM](#derived-table-subquery-in-from)
- [Aggregation Patterns](#aggregation-patterns)
  - [GROUP BY with HAVING](#group-by-with-having)
  - [GROUPING SETS — multiple grouping levels](#grouping-sets-multiple-grouping-levels)
  - [ROLLUP — hierarchical subtotals](#rollup-hierarchical-subtotals)
  - [CUBE — all combinations](#cube-all-combinations)
  - [FILTER clause (PostgreSQL) — conditional aggregation](#filter-clause-postgresql-conditional-aggregation)
- [UPSERT Patterns](#upsert-patterns)
  - [PostgreSQL — ON CONFLICT](#postgresql-on-conflict)
  - [MySQL — ON DUPLICATE KEY](#mysql-on-duplicate-key)
  - [SQL Server — MERGE](#sql-server-merge)
- [JSON Operations](#json-operations)
  - [PostgreSQL JSONB](#postgresql-jsonb)
  - [MySQL JSON](#mysql-json)
- [Pagination Patterns](#pagination-patterns)
  - [Offset pagination (simple but slow for deep pages)](#offset-pagination-simple-but-slow-for-deep-pages)
  - [Keyset pagination (fast, requires ordered unique column)](#keyset-pagination-fast-requires-ordered-unique-column)
  - [Keyset with composite sort](#keyset-with-composite-sort)
- [Bulk Operations](#bulk-operations)
  - [Batch INSERT](#batch-insert)
  - [Batch UPDATE with VALUES](#batch-update-with-values)
  - [DELETE with subquery](#delete-with-subquery)
  - [COPY (PostgreSQL bulk load)](#copy-postgresql-bulk-load)
- [Utility Patterns](#utility-patterns)
  - [Generate series (PostgreSQL)](#generate-series-postgresql)
  - [Deduplicate rows](#deduplicate-rows)
  - [Pivot (manual)](#pivot-manual)
  - [Conditional INSERT (skip if exists)](#conditional-insert-skip-if-exists)
