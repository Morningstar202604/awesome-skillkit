...
def score_case(case: dict, weights: dict, forbidden: list) -> dict:
    ...
    if evidence:
        ...
        grnd = min(1.0, hits / max(1, len(evidence)) * 2.0)  # half the evidence = full marks
    else:
        grnd = 1.0 if response else 0.0  # no evidence declared -> any output has no hallucination risk
    grnd = round(grnd, 3)
...
def main() -> int:
    ...
    ap.add_argument("--forbidden-json", help="append a forbidden-string list (JSON array)")
    ap.add_argument("--out", default="eval_result.json", help="result output path")
    ap.add_argument("--force", action="store_true")
    ap.add_argument("--write", action="store_true", help="actually write to disk; default dry-run")
    args = ap.parse_args()
