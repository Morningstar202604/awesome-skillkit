...
- [三、表演与微表情细节](#三表演与微表情细节让角色活而非动)——让角色"活"而非"动"
- [四、速度与节奏词](#四速度与节奏词motion-槽位)——motion 槽位
- [五、运动类型词](#五运动类型词runway-官方-movement-types画面整体的变形方式)——Runway 官方 movement types
- [六、物理属性描述法](#六物理属性描述法可灵写实模型强项)——可灵/写实模型强项
- [七、各模型方言速查](#七各模型方言速查同一个镜头不同模型说法不同)——同一个镜头，不同模型说法不同
- [八、迭代修复对照表](#八迭代修复对照表哪坏了改哪个槽)——哪坏了改哪个槽
- [九、负面清单](#九负面清单写了必崩的)——写了必崩的
