...
SCRIPT = Path(__file__).resolve().parent / "subtitles.py"
...
def test_missing_script_file_clean_error():
    import subprocess, sys, json
    r = subprocess.run([sys.executable, str(SP / "subtitles.py"), "--script", "/no/such/s.json"],
                       capture_output=True, text=True, timeout=60)
...
def test_bad_json_script_clean_error(tmp_path):
    import subprocess, sys, json
    bad = tmp_path / "bad.json"
    bad.write_text("{oops", encoding="utf-8")
    r = subprocess.run([sys.executable, str(SP / "subtitles.py"), "--script", str(bad)],
                       capture_output=True, text=True, timeout=60)
    assert r.returncode == 2, r
