...
若失败：尺寸校验不通过 → 按 16 倍数/宽高比 1:3~3:1/总像素区间三条规则取整后重跑，不要带病进入步骤 2；脚本入口报错（`--prompt` 缺失）→ 补 prompt 后重跑。
...
若失败：服务不可达 exit 1 → 提示用户启动 127.0.0.1:30080 的服务后重跑，**不要伪造占位图**；轮询超时 → 调大 `--timeout` 重跑，仍无结果则把 task_id 报给服务维护方；`--jpg` 压缩失败 → `pip install pillow` 后重跑，或去掉 `--jpg` 只交 PNG；下载 0 字节 → 重新轮询取新 `result_url` 再下载。
...
预期：prompt 覆盖主体/风格/光影/构图/配色五要素，尺寸已对上目标平台。
若失败：目标平台不在表内 → 按其官方规范取尺寸并核对 16 倍数与像素区间，核不上就取最接近的合法尺寸并告知用户；prompt 里想写中文字符 → 改为只描述画面、文字留给后期（模型出中文易乱码，见失败处置表）。
