...
* Refactoring: pull latest main first, work on a fresh branch, keep commits atomic with messages that state the why, and always run the full check suite before pushing (for this repo: `python -m pytest skills -q`, `python tools/validate_skills.py`, and `python build.py`). A branch left behind main cannot be merged under the repository's branch protection.
